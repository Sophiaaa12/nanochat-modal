...
def stage_pretrain_swiglu(
    depth: int = DEPTH,
    device_batch_size: int = DEVICE_BATCH_SIZE,
    wandb_run: str = "picochat_swiglu",
) -> None:
    """
    Pretrain with SwiGLU MLP (ablation). Requires gpt_swiglu.py and
    scripts/base_train_swiglu.py to be in the nanochat/ copy before image build.
    Same as stage_pretrain but runs scripts.base_train_swiglu with model-tag d12_swiglu.
    """
    _setup_cache()
    print("Resetting training report...")
    _python("nanochat.report", ["reset"])
    print(
        f"Starting SwiGLU pretraining: depth={depth}, "
        f"device_batch_size={device_batch_size}, nproc={_N_PRETRAIN_GPUS}, run={wandb_run}"
    )
    model_tag = f"d{depth}_swiglu"
    _torchrun(
        "scripts.base_train_swiglu",
        [
            f"--depth={depth}",
            f"--device-batch-size={device_batch_size}",
            f"--run={wandb_run}",
    ...
    volume.commit()
    print("SwiGLU pretraining complete.")
...
def stage_post_pretrain_eval(model_tag: str = "d12") -> None:
    """
    Evaluate the base model immediately after pretraining.

    speedrun.sh: torchrun ... -m scripts.base_eval -- --device-batch-size=16

    scripts.base_eval (default --eval core,bpb,sample) computes:
        - val bpb (validation bits per byte) on held-out data
        - CORE metric: zero-shot across 22 benchmarks (HellaSwag, ARC, ...)
    The eval bundle (~1GB) is downloaded on first run and cached in the volume.

    Pass model_tag to choose which checkpoint to evaluate. When both d12 (baseline)
    and d12_swiglu exist, the default is "d12". Use model_tag="d12_swiglu" only if
    you have a SwiGLU-capable eval path (base_eval loads standard GPT, not GPTSwiGLU).
    """
    _setup_cache()
    ...
    # Explicit --model-tag avoids guessing when both d12 and d12_swiglu exist (else picks d12).
    print(f"Running base_eval for model_tag={model_tag} (val bpb + CORE metric, ~20-40 min)...")
    _torchrun(
        "scripts.base_eval",
        ["--device-batch-size=16", f"--model-tag={model_tag}"],
        nproc=_N_PRETRAIN_GPUS,
    )
...
def show_eval_csv(rel_path: str = "base_eval/base_model_002205.csv") -> None:
    """
    Read and print a CSV from the nanochat Volume (e.g. base_eval output).

    Usage:
        uv run modal run nanochat_modal.py::show_eval_csv
        uv run modal run nanochat_modal.py::show_eval_csv --rel-path base_eval/other_file.csv
    """
    volume.reload()  # get latest from Modal
    path = os.path.join(NANOCHAT_CACHE, os.path.normpath(rel_path))
    if not os.path.isfile(path):
        base = os.path.join(NANOCHAT_CACHE, "base_eval")
        try:
            names = os.listdir(base)
        except FileNotFoundError:
            names = []
        print(f"File not found: {path}")
        print(f"Existing files in nanochat_cache/base_eval: {names}")
        return
