# Ablation SwiGLU — Modal runs

Code to run nanochat picochat (d12) baseline and SwiGLU ablation on Modal.  
Uses the same `nanochat_modal.py` and ablation code as the tutorial repo.

## What’s in this repo

- **`nanochat_modal.py`** — Modal app (stage_data, stage_tokenizer, stage_pretrain, stage_pretrain_swiglu, stage_post_pretrain_eval, etc.)
...
- **`WANDB_RUN`** — set to `"picochat_baseline"` for Run 1, `"picochat_swiglu"` for Run 2.

Checkpoints and data live on the Modal Volume **nanochat-vol**; no need to re-download for Run 2.
